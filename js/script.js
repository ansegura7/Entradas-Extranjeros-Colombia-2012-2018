...
ast.createBarChart = () => {
	...
	if (chartType == "month")
		...
		multi1 = 1;
	//console.log("chartType: " + chartType + ", multi1: " + multi1 + ", multi2: " + multi2);
	...
	// Aggregate data
	let aggData = ast.aggregateDataByField(filterData, "nationality", "total", multi1);
	let sortData = ast.getTableData(aggData, chartType, multi2);
...
// Create table data
ast.getTableData = (data, dataType, multi) => {
	let tbData = [];
	monthlyValue = 0;
	yearlyValue = 0;
	...
	data.forEach(function(d) {
		if (dataType == "month") {
			monthlyValue = +ast.toFixedNumber(d.total, 1, 2);
			yearlyValue  = +ast.toFixedNumber(d.total, (1 / multi), 2);
		}
		else {
			monthlyValue = +ast.toFixedNumber(d.total, multi, 2);
			yearlyValue  = +ast.toFixedNumber(d.total, 1, 2);
		}

		let r = [0, d.nationality, monthlyValue, yearlyValue];
		tbData.push(r);
	});
